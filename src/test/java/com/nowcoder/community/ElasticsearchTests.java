...
public class ElasticsearchTests {
    ...
    @Autowired
    private DiscussPostMapper discussMapper;//从数据库中查询帖子

    @Autowired
    private DiscussPostRepository discussRepository;//es中查询数据
...
/*构造搜索条件*/         .withQuery(QueryBuilders.multiMatchQuery("互联网寒冬","title","content"))
/*构造排序条件1*/          .withSort(SortBuilders.fieldSort("type").order(SortOrder.DESC))
/*构造排序条件2*/          .withSort(SortBuilders.fieldSort("score").order(SortOrder.DESC))
/*构造排序条件3*/          .withSort(SortBuilders.fieldSort("createTime").order(SortOrder.DESC))
/*构造分页条件*/           .withPageable(PageRequest.of(0,10))
/*构造高亮显示条件*/        .withHighlightFields(
/*构造高亮显示字段、前缀标签、后置标签*/new HighlightBuilder.Field("title").preTags("<em>").postTags("</em>"),
/*构造高亮显示字段、前缀标签、后置标签*/new HighlightBuilder.Field("content").preTags("<em>").postTags("</em>")
                ).build();//此方法执行则返回searchQuery方法的实现类
//底层通过调用 elasticTemplate.queryForPage(searchQuery,class,SearchResultMapper)
//底层获取到了高亮显示的值,但是没有返回
    ...
    public void testSearchByTemplate() {
        ...
        Page<DiscussPost> page=elasticTemplate.queryForPage(searchQuery, DiscussPost.class, new SearchResultMapper() {
            ...
            public <T> AggregatedPage<T> mapResults(SearchResponse response, Class<T> aClass, Pageable pageable) {
                //通过response取到此次搜索到的数据
                SearchHits hits = response.getHits();//多条数据
